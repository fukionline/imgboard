...
#
#
#     画像アップロード掲示板 携帯アクセスCGI(v)
#
#       im.cgi v1.22 (imgboard Rev.6 Base)
#
#	 Copyright(C)1998-2010	1998特報!!倶楽部
...
#  2010/10/05 docomoフルブラウザでページ送り用ボタンが画面右端に消える現象に対処
#  2010/08/18 外部ファイル、スパム単語リスト(spamword.cgi)の読み込みに対応
#  2010/05/27 ustream.tvのライブ埋込み再生に対応
#  2010/04/24 tokenのタイムアウトエラーが出る不具合を修正
#  2010/04/09 ワード検索メニューにおけるクロスサイトスクリプティングの可能性に対し対策をした
#  2010/04/05 SoftBank desire対策をした
#  2010/03/31 高負荷サイト用HTMLキャッシュ出力をONにするとSPAM対策用ワンタイムトークンでエラーが出る問題を修正
#  2010/03/10 SPAM対策ソフトのバグを修正
#  2010/02/17 トリップ機能(なりすまし防止）をテスト実装
#  2010/02/15 SPAM対策にワインタイムトークン機能を追加
#  2010/01/23 android用のCGIコードを追加
#  2010/01/23 android Ver1.6のUSERAGENTが携帯と誤認識される問題でimode判定部を修正
#  2009/12/18 電子書籍のepub形式に対応
#  2009/12/18 記事中の住所を認識し、GoogleMapリンク化する機能を追加
#  2009/12/18 デコメ、住所録データ等への対応を追加
#  2009/12/18 中国語圏からのSPAM対策を強化
#  2009/12/18 動画共有サイトdailymotion.comの埋込みに対応
#  2009/12/08 docomoCHTML7.2以降(906～)とSoftbank,au最新機種でブラウザから直接投稿可能に
#  2009/12/08 iphoneとimode2.0,Softbank,auの現行機種でcookie有効に。入力が楽に
#  2009/12/08 機能見直し:imode2.0でcookieが実装された。icookie機能を廃止。
#  2009/12/08 サービス終了に伴ないEzweb用のHDML部分、 H"対応部分を削除
...
#  2008/06/16 iPhone/iPodの画像アップロード用コードを追加
#  2008/05/28 youTube動画の一部(IDに _ が入ったもの等)が携帯で視聴できない点を修正
#  2008/04/07 youtube(jp)の携帯視聴URLに対応
#  2007/06/05 自称されたメールアドレスの国チェック機能を追加
#  2007/05/18 @写メール終了に伴い,スクリプト整理
#  2006/12/13 Softbank,auでFlashを見られるようにした
#  2006/10/21 SoftBank対応
#  2006/10/21 youTubeタグ対応
#  2006/03/27 掲示板SPAM対策を追加
#  2005/01/16 au_3Gで１ページ当たりの記事数指定が無効だったを修正
#  2004/12/12 sage機能を追加
...
#==================================#
#
$PM{'admin_passwd'} = '1464';		# 管理人による記事削除時のパスワード
#					# (変更してください)
#
...
#  ご認識されて自由な会話ができない場合、こちらを使ってみてください。
#
#  (1=制限する(推奨),0=制限しない)
...
#
#  サイズ制限(画像)
#  デフォルト500ＫＢ(埋めこみ表示が遅くなるので500KB以上にはしないこと。)
$PM{'max_upload_img_size'} 	= 500;	# 単位KB
#
#  ■<各種マルチメディアデータ（約２０種類）のアップロード>
#
#  デフォルトではGIF/JPEG/PNG/3GP動画/ezmovie/
# 写メール(JPEG)/WMV/MP4/MP3/WMA/PDFの投稿を受け付け、それ以外のデータは自動リジェクトします。
# その他の各種マルチメディアデータ
...
$google_ad01
<CENTER>
(携帯ｱｸｾｽ) -$ACCESS_COUNTER{total}-<BR>
</CENTER>
...
HTML_END
    ...
    if($LDATA{'img_location'} ne ""){
	  if($can_handle_flag == 1 ){	# 扱える場合はリンク表示
       #2009.12.07 iPod/iPhoneでフルスクリーン化
...
print<<HTML_END;
$data_type： $tttmp_imgtitle-$IMG_PARAMETERS{'dsize'}
HTML_END

	  } #if($can_handle_flag == 1 )
 
	} # $LDATA{'img_location'}がある
...
# バナー１つ目（変更不可）
$B_BANNER{'1'}=qq|
<A HREF="http://www.big.or.jp/~talk/welcome/welcome_imr7.cgi">[(広)NEW!ガラ携.写ﾒUP対応imgboard2015はココ]</a>
|;
...
# R7new
	...
	&check_upload_from_pc;
if($au_3G_flag >= 1){ 
&error("（アドバイス）auの3G携帯の場合、EZwebブラウザでなく、PCサイトビューアーを使い、PC用の方の掲示板へアクセスすると画像を簡単にアップロードできます（2007春以降モデル。ただし、アップロードは最大80KBなので投稿前に縮小作業必要）。なお、メール添付方式だと、より古い機種でも投稿可\能\ですが、この場合、メール添付投稿に対応したより新しい掲示板スクリプトimgboard 2010以降が必要です．このバージョン(imgboard R6系)では投稿できません．");
}elsif($HTTP_USER_AGENT =~ /^DoCoMo/i){
&error("メール添付による写真投稿機能\を使うためには、メール添付投稿に対応したより新しい掲示板スクリプトimgboard 2010以降が必要です．このバージョン(R6系)では投稿できません．（アドバイス）906i/706i以降の場合、1.1の投稿方法をお選びください。それ以前のdocomoのFOMA携帯の場合、フルブラウザを使うと画像をアップロードできる場合があります。フルブラウザ携帯でアクセスし直してみてください ");
}else{
&error("メール添付による写真投稿機能\を使うためには、メール添付投稿に対応したより新しい掲示板スクリプトimgboard 2010以降が必要です．このバージョン(R6系)では投稿できません．");
}
	&output_Content_type;
	&top_html;
				...
				if($PM{'max_upload_img_size'} ne ""){
				 if($ENV{'CONTENT_LENGTH'} >$PM{'max_upload_img_size'}*1024){
				  if($new_fname=~ /\.gif$|\.jpe?g$|\.png$|\.bmp$/i){
					&error(" デ\ータが大きすぎます。画像サイズは$PM{'max_upload_img_size'} KB以下にしてください。カメラの撮影設定を見直し、撮影解像度を下げてください。 ");
					exit;
				  }
				 }
				}
...
sub output_form_html{
	...
	&form_imode_html;	# フォーム(imodeに統一2009.12)
...
HTML_END
	...
	if($start_message > 1){
		...
		print " <a href=\"$cgi_name?page=$pre_message&mode=$FORM{'mode'}&bbsaction=page_change$mes_p1\"> 前$PM{'message_per_page'}</a>\n";
	...
	if($all_message > $last_message){
		...
		print " <a href=\"$cgi_name?page=$next_message&mode=$FORM{'mode'}&bbsaction=page_change$mes_p1\"> 次$PM{'message_per_page'}\> </a>\n";
...
[Imgboard - Mes]<BR>
 ...
 $mes_01 $mes_02 <BR>
$accesskey_p1<a href="$cgi_name?page=$FORM{'page'}&viewpass=$FORM{'viewpass'}" accesskey=0>掲示板へ戻る</a>
</BODY>
</HTML>
HTML_END
...
#	$ext{'image/bmp'}	= 'bmp';	# for BMP  file
	...
	$ext2{'x-shockwave-flash'}= 'swf';	# for Shockwave_flash(505i用)
...
$HR
・<a href="$cgi_name?page=$FORM{'page'}&mode=user_01"> 携帯おリンク! </a><BR>
...
$HR
(添付ﾌｧｲﾙ)<BR>
デフォルトではGIF/JPEG/PNG/MNG/3GP動画/デコメ/電話帳/WMV/WMA/PDF/MP4/ezmovie/ASF許可です。それ以外の各種マルチメディアデータ（テキスト,HTML,着メロなど約３０種類）の投稿を許可する場合は以下のフラグを1にしてください。<BR>
HTML_END
...
<P>
(画像/動画投稿)<BR>
51,52,53ｼﾘｰｽﾞ以降のJ-SKY ﾊﾟｹｯﾄ対応\機\(ﾑｰﾋﾞ写ﾒｰﾙ機)は直接写真をｱｯﾌﾟﾛｰﾄﾞ可\能\です。<BR>なお、その他の写メール機、EZのフォトメール、i-shot等からは画像を投稿できません。これらのｶﾒﾗ付き携帯に関しては、現在開発中のスクリプト「全ｶﾒﾗ付き携帯対応のimgboard 2010」からの対応になりますので、それをお使いください。
...
<BR>
画像Upload掲示板(携帯ｱｸｾｽCGI)は、imgboard運用者やそのお友達のﾕｰｻﾞ方々からの要望に応えて、ガラパゴス携帯用に開発した追加ｽｸﾘﾌﾟﾄです。なお、このｽｸﾘﾌﾟﾄは、基本的には単体では動きませんので、imgboardと組合わせてご使用ください。
<BR>
...
<INPUT TYPE="hidden" NAME="parent" VALUE="$FORM{'parent'}">
<BR>
1.2ﾒｰﾙ添付2段式$mes_p2<BR>
<BR>
<INPUT TYPE="submit" VALUE=" GO ">
</FORM>
<BR>
1.3 フルブラウザ式(iPhone/iPad/iPod)<BR>
<a href="$PM{'cgi_hontai_name'}" ifb="$PM{'cgi_hontai_name'}">フルブラウザ </a> をご利用ください。<BR>
<BR>
<BR>
<BR>
HTML_END
...
<BR>
<CENTER>
$accesskey_p1<a href="$cgi_name?page=$FORM{'page'}&viewpass=$FORM{'viewpass'}" accesskey=0>中止</a>
</CENTER>
$HR
</BODY>
</HTML>
