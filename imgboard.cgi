...
#  2014/02/11 iOS7に対するアップデート修正（文言修正程度）
...
#  2012/05/07 ユーザiconの上限を10から150に増やした
...
#  2011/12/15 Androidスマフォから直接2MB級の投稿を受け付けるオプションを追加
#  2011/12/15 XSS（クロススクリプティング）対策で修正
#  2011/12/15 Androidスマフォから直接2MB級の投稿を受け付けるオプションを追加
#  2011/11/20 youtubeの新埋め込みURL「youtu.be」対応
#  2011/11/20 ニコニコ動画の「動画情報」iframe埋め込みURL対応
#  2011/06/10 youtube,dailyMotion,ustream,nicovideoがiframeに移行したので対処
#  2011/06/10 youtubeのAPI変更に伴いfmt指定機能を廃止
#  2011/06/08 twitterの新APIにあわせてパラメータを変更
#  2011/02/05 ニコニコ動画外部プレイヤーに対応（埋め込み再生できます）
#  2011/02/04 Android2.1ではFlash非対応とする判定処理追加
...
#  2010/03/30 コメントが記事に回り込む昔のレイアウトを復活させた
#  2010/03/28 iPhone/iPadのタッチパネル操作用にボタンのサイズを大きくした
#  2010/03/28 SPAM予防のため、emailを入れる人が少ないので、デフォルトで省略とした
#  2010/02/22 エスケープ機能を使ったSPAM対策を追加
#  2010/02/17 トリップ機能(なりすまし防止）をテスト実装
#  2010/02/15 SPAM対策にワインタイムトークン機能を追加
#  2010/02/15 IPアドレス指定型SPAMフィルタ機能を追加
#  2010/02/05 SPAM広告リストを大幅更新し、SPAM投稿対策を強化
...
#  2009/12/18 著作権に対する注意文に、ダウンロード行為を追記（法改正）
...
#  2007/05/20 題名やメール欄にURLリンクを埋め込めなくした(SPAM対策)
#  2007/05/18 3gppファイルのPC再生リンク先がpocketPC用ソフトだった。修正
#  2006/12/13 FLV(Flash Video)動画データに対応
#  2006/12/13 アップロード上限値を拡大(30MB)
#  2006/10/23 SPAMワードのデフォルトから「動画」「新着」等10個を削除した。
#  2006/10/20 SoftBankモバイル対応
#  2006/06/29 掲示板SPAM対策でURLリンクの個数を3個までに制限する機能を追加
#  2006/05/01 正規投稿,SPAMの誤認識防止目的で、SPAM_WORD指定欄を追加
#  2005/01/15 画像アップ上限を越えると、縮小ソフトを案内するようにした
#  2005/01/09 m4aファイル(MPEG-4 AAC Audio)対応
...
$mail_prog = '/usr/lib/sendmail'; # 古いサーバでのパス
#$mail_prog = '/usr/sbin/sendmail'; # 最近がこっちのパスが多いです
#
...
#
#  <フォーム入力部を掲示板上に表示>
$form_disp_on_board	=1;		# (1=yes,0=no)デフォルトは1
#
...
<font size=-1>
 <UL>
 <LI>パ\ソ\コ\ン、iPhone、Android、携帯上の$html_mA01を,アップロードできます．($mes_01画像最大<B>$max_upload_img_size KB(Android可)</B>/画像以外<B>$max_upload_size KB</B>まで)
...
$iPhone_mes_01=qq|
<font color=red>(iOS.Ver.5以下のiPhone/iPadで画像投\稿\)</font><BR>
 (方法\.1)iOS.Ver.6以降にアップデートするとsafariで直接投\稿\できます<BR>
 (方法\.2)iOS.Ver.5以下ユーザ：<a href="http://itunes.apple.com/jp/app/simple-resize/id327776379?mt=8&uo=4" target="itunes_store">Simple Resize</a>で写真を縮小(640サイズ以下厳守)。<a href="http://itunes.apple.com/jp/app/opera-mini-web-browser/id363729560?mt=8&uo=4" >Opera Mini</a>(File添付OK)でWebアクセスし投\稿\! 
|;
}else{
$iPhone_mes_01=qq|
...
#		}
	...
	if($limit_wp_max_message ne ""){
	  if(($limit_wp_max_message-1) < $all_message){
	    &error(" エラー。imgboardでは、Webパーツは最大 $limit_wp_max_message までしか保存できません。<P>それ以上のパーツやアイコンを保存したい場合は、姉妹スクリプトimgboard2010をご利用ください。 ");
	  }
	}
